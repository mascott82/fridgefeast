...
const FavouriteButton = ({ addNew, userid, recipeid, onClick }) => {
  const clickAction = (event) => {
    event.preventDefault() // Prevent default behavior of the link
    const userRecipeTarget = { userid: userid, recipeid: recipeid }
    const url = addNew
      ? backend_endpoint_user_fav_add
      : backend_endpoint_user_fav_delete
    axios
      .post(url, userRecipeTarget)
      .then((response) => {
        const qty = response.data.qty
        console.log("change-fav-qty", qty)
        if (onClick) onClick() // If onClick prop is provided, call it after successful request
      })
      .catch((error) => {
  ...
  return (
    <button onClick={clickAction} style={{ backgroundColor: "transparent" }}>
